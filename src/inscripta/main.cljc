(ns inscripta.main
  "Kotodama WASM entrypoint for the Inscripta clean-room actor (L4) — Clojure port.
  ...
  facts (`inscripta.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "inscripta")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"inscripta.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "inscripta-compat" :tier tier :entities entities} 200])
...
(defn start! [] :inscripta-compat/ready)
